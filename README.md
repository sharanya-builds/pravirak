...
You need two terminals (or a process manager) — one for each service.

**1. Backend**

```bash
cd backend
docker compose up -d      # starts a local PostgreSQL 16
npm install
cp .env.example .env
...
Open http://localhost:5173. You'll land on the Login page — log in, create an
account, or choose "Continue as Guest."

### Optional: live Google Maps

Set `VITE_GOOGLE_MAPS_API_KEY` in `frontend/.env` to enable live Google Places
address search and map rendering during location selection. Without a key, the app
falls back to a built-in demo dataset of Indian localities so the full flow still
works end-to-end.
...
cd backend && npm start         # serves the API; point your frontend host / reverse
                                   # proxy at it, or set VITE_API_BASE_URL accordingly
```
